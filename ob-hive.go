...
type ObHive struct {
	...
	//	This is never nil; if Opt.Sandboxed is true, this is a no-op drop-in dummy.
	Watch *uio.Watcher
}
...
func (_ ObHive) IsHive(dirPath string) bool {
	return uio.DirsFilesExist(dirPath, "cust", "dist")
}

//	Returns a cleaned Hive-relative path for the specified path segments.
//	For example, if Hive.DirPath is "obtest/hive", then Hive.Path("pkg", "mysql") returns "obtest/hive/pkg/mysql"
